...
public class dbConnect
{
  ...
  public dbConnect(String user,String pass)
  {
	try
	{
	  ...
	  con = DriverManager.getConnection("jdbc:mysql://localhost:3306/Sy2017_18_1stsem", user, pass);
  ...
  public void createTable(String dName)
  {
	try
	{
	  ...
	  st.execute(tableMaker);
	  tableMaker = "create table grades (gId int not null auto_increment, prelim int, midterm int, final int, foreign key (gId) references enrolled(eID))";
	  st.execute(tableMaker);
  ...
  public String[][] getEnData(String StudID)
  {
	try
	{
	  ...
	  String query = "SELECT subject.subjID,subject.subjCode,subject.sched,enrolled.studID FROM enrolled INNER JOIN subject ON subject.subjId=enrolled.subjId where enrolled.studId =" + StudID;
	  rs = st.executeQuery(query);
	  int z = 0;
	  while (rs.next())
	  {
  ...
  public void updateData(String name, String address, String Corse, String gender, String year)
  {
	try
	{
	  String query = "update student set addr='" + address + "',course='" + Corse + "',gender='" + gender + "',yrlevel='" + year + "' where name='" + name + "'";
	  st.executeUpdate(query);
